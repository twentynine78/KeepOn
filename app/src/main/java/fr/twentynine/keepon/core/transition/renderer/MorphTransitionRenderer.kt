...
class MorphTransitionRenderer(private val transition: MorphTransition) : IconTransitionRenderer {
    ...
    override fun prepare(from: Bitmap, to: Bitmap): TransitionFrames {
        ...
        val scale = fromMetrics.inkHeight / toMetrics.inkHeight
    ...
    private fun metricsOf(alpha: FloatArray, width: Int, height: Int): GlyphMetrics {
        ...
        if (mass <= 0f) return GlyphMetrics(width / 2f, height / 2f, height.toFloat())
        val inkHeight = (maxY - minY + 1).toFloat().coerceAtLeast(1f)
        return GlyphMetrics(sumX / mass, sumY / mass, inkHeight)
    }

    private class GlyphMetrics(val cx: Float, val cy: Float, val inkHeight: Float)
}
